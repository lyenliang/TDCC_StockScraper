...
def reset_price_table
    ...
    @client.query("CREATE TABLE #{@stock_price_table_name} (stock_number VARCHAR(255), date DATE, closing_price FLOAT);")
end
...
def insert_price_table(stock_number, date, closing_price)
    @client.query("INSERT INTO #{@stock_price_table_name} (stock_number, date, closing_price)
            VALUES ('#{stock_number}', '#{date}', '#{closing_price}');")
end
...
def fetch_otc(stock, date)
    ...
    for i in 5..(rows.size-2)
        ...
        insert_price_table(stock, date, price)
    end
    return true
end
